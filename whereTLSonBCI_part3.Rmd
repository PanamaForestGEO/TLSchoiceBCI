...
The objective of this document is to explore potential locations for three (or possibly more) 1-ha plots within the BCI 50-ha plot to be scanned with a Terrestrial Laser Scanner (TLS) for GEOTREES.  These plots should represent the variation in forest structure within the site.  Ideally forest structure is relatively homogenous within the chosen plots, and overall the set of plots spans the variation in forest structure within the entire 50-ha plot, but with a bias towards capturing more bigger trees.  
...
### Calculate forest structure stats by 20 x 20 quadrat 
...
Make figures of habitat heterogeneity.  
...
Make figures of AGB density at 1-ha scale  
...
Make figures of BA density for 1-ha areas centered on each quadrat.
...
Get the N>10 density for 1-ha areas centered on each quadrat and make figures of it.  
...
Get 1-ha means of canopy height and make maps of these.  
...
Now run these functions for variables of interest.  We want to subplots to span plot-level variation in the following mean variables (so better if more different in pdfs and cdfs).  
...
Use the same functions to check heterogeneity within subplots.  We want the chosen subplots to show low variation within subplots.  They don't need to be the lowest possible, but they shouldn't be in the highest range of within-plot variation.  
...
Show positions of these plots in scatterplots of pairs of variables, with symbols indicating habitat type
...
```{r show-new-in-scatterplots}

par(mar=c(5,4,4,8),xpd=TRUE)
plot(n10use,bause,xlab="N >= 10 cm in 1-ha subplots in 2015",
     ylab="Basal area in 1-ha subplots in 2015",cex=0.5,pch=16,col="grey")
points(newtlscenters$n10,newtlscenters$ba,col=cols,pch=16)
legend("topright",inset=c(-0.2,0),legend=seq(1,nnew),pch=16,col=cols[1:nnew])

plot(n10quant,baquant,xlab="Quantile N >= 10 cm in 1-ha subplots in 2015",
     ylab="Quantile Basal area in 1-ha subplots in 2015",cex=0.5,pch=16,col="grey")
points(newtlscenters$n10quant,newtlscenters$baquant,col=cols,pch=16)
legend("topright",inset=c(-0.2,0),legend=seq(1,nnew),pch=16,col=cols[1:nnew])


par(mar=c(5,4,4,8),xpd=TRUE)
plot(n10use,agbuse,xlab="N >= 10 cm in 1-ha subplots in 2015",
     ylab="AGB in 1-ha subplots in 2015",cex=0.5,pch=16,col="grey")
points(newtlscenters$n10,newtlscenters$agb,col=cols,pch=16)
legend("topright",inset=c(-0.2,0),legend=seq(1,nnew),pch=16,col=cols[1:nnew])

plot(n10quant,agbquant,xlab="Quantile N >= 10 cm in 1-ha subplots in 2015",
     ylab="Quantile AGB in 1-ha subplots in 2015",cex=0.5,pch=16,col="grey")
points(newtlscenters$n10quant,newtlscenters$agbquant,col=cols,pch=16)
legend("topright",inset=c(-0.2,0),legend=seq(1,nnew),pch=16,col=cols[1:nnew])


par(mar=c(5,4,4,8),xpd=TRUE)
plot(canhtuse,agbuse,xlab="Mean Canopy Height in 2023",
     ylab="AGB in 1-ha subplots in 2015",cex=0.5,pch=16,col="grey")
points(newtlscenters$canht,newtlscenters$agb,col=cols,pch=16)
legend("topright",inset=c(-0.2,0),legend=seq(1,nnew),pch=16,col=cols[1:nnew])

plot(canhtquant,agbquant,xlab="Quantile Mean Canopy Height in 1-ha subplots in 2015",
     ylab="Quantile AGB in 1-ha subplots in 2015",cex=0.5,pch=16,col="grey")
points(newtlscenters$canhtquant,newtlscenters$agbquant,col=cols,pch=16)
legend("topright",inset=c(-0.2,0),legend=seq(1,nnew),pch=16,col=cols[1:nnew])
...
Map locations of these plots relative to variables of interest

```{r show-proposed-1ha}
image.plot(q20x,q20y,cagb2015mbyq20,main="Crown-distributed 20x20 AGB density (Mg/ha) in 2015 & Habitat type")
for (i in 1:nrow(newtlscenters)) {
  lines(x=rep(newtlscenters$x[i]-50,2), y=c(newtlscenters$y[i]-50,newtlscenters$y[i]+50),lwd=3,lty=3,col="forest green")
  lines(x=rep(newtlscenters$x[i]+50,2), y=c(newtlscenters$y[i]-50,newtlscenters$y[i]+50),lwd=3,lty=3,col="forest green")
  lines(x=c(newtlscenters$x[i]-50,newtlscenters$x[i]+50), y=rep(newtlscenters$y[i]-50,2),lwd=3,lty=3,col="forest green")
  lines(x=c(newtlscenters$x[i]-50,newtlscenters$x[i]+50), y=rep(newtlscenters$y[i]+50,2),lwd=3,lty=3,col="forest green")
...
image.plot(q20x,q20y,sqrt(cagb2015mbyq20),main="Sqrt Crown-distributed 20x20 AGB density (Mg/ha) in 2015 & All subplots")
for (i in 1:nrow(dend40centers)) {
lines(rep(dend40centers$x[i]-20,2),c(dend40centers$y[i]-20,dend40centers$y[i]+20))
lines(rep(dend40centers$x[i]+20,2),c(dend40centers$y[i]-20,dend40centers$y[i]+20))
lines(c(dend40centers$x[i]-20,dend40centers$x[i]+20),rep(dend40centers$y[i]-20,2))
...
```
# Canopy height with proposed plots 

```{r canht-1ha}
#image.plot(xs,ys,lidar2023rev2,col=gray.colors(20),main="Lidar canopy height May 2023 and proposed 1-ha plots ",xlab="",ylab="")
image.plot(x5s,y5s,mnch23p5,col=heat.colors(20),
           main="5-m Mean Lidar canopy height May 2023 and proposed 1-ha plots",xlab="",ylab="")
lines(c(0,0,1000,1000,0),c(0,500,500,0,0))
...
# Liana stem map and proposed plots
...
# Canopy height and liana stem map and proposed plots 

```{r canht-liana-plots}

#image.plot(xs,ys,lidar2023rev2,col=gray.colors(20),main="Canopy height May 2023 and Liana stems and Proposed 1-ha",xlab="",ylab="")
image.plot(x5s,y5s,mnch23p5,col=gray.colors(20),
           main="5-m Mean Lidar canopy height May 2023 and Liana stems and Proposed 1-ha",xlab="",ylab="")
#image.plot(xs,ys,lidar2023rev2,col=gray.colors(20),main="Canopy height May 2023 and Liana stems and Proposed 1-ha",xlab="",ylab="")
rasterImage(lianamap4,xleft=-offset,ybottom=-offset,xright=1000+offset,ytop=500+offset)
lines(c(0,0,1000,1000,0),c(0,500,500,0,0))
#points(bci_habitat$x,bci_habitat$y,pch=as.numeric(as.factor(bci_habitat$habitat)),cex=0.6)
