---
title: 'where to place TLS plots in BCI: (2) calculate quadrat-level stats for BCI 50-ha tree census'
author: "Camille Piponiot and Helene Muller-Landau"
date: "2023-09-12"
...
This code was adapted by Helene Muller-Landau from code written by Camille Piponiot for the following book chapter:  
...
This assumes have already run "whereTLSonBCI_part1a.Rmd" to download the data, and have it in the appropriate repository.
...
req_packages <- c("rdryad", "data.table", "utils", "truncnorm", "openxlsx", "docxtractr","readxl")
...
#1, 10, 20, 30, 50,
## above 50 cm dbh
#df_stem_melt_n50 <- df_stem_melt[method=="chave14_h" & DFstatus=="alive"& ! size %in% 
#                  c("[1,10]","(10,30]","(30:50]))]
#df_stem_melt_n50[, `:=`(variable = "stock", method = "n50", value = 1, stemID = 0)] 
#df_stem_melt_n50 <- unique(df_stem_melt_n50)
#df_stem_melt <- rbind(df_stem_melt, df_stem_melt_n50)
...
Below is a function that aggregates AGB and AWP values by quadrat, with the option to apply the correction from @Kohyama2019 and returns them by quadrat.   


```{r aggr-agb-dyn-byquad_function}
agb_dyn_byquad <- function(data = df_stem_melt, 
...
#                         measure.vars = grep("gain|loss|stock|delta", 
#                                             colnames(df), value = TRUE), 
#                         id.vars = c("quadrat","group", "year"))
...
### By 20 x 20 Quadrat (Code added by Helene)

We first estimated quadrat-level annual AGB and AWP.
...
df_quad_kohyama <- agb_dyn_byquad(
  group = "method",
  area = 0.04,
  kohyama = TRUE, 
  nboot = 1000, 
...
df_quad_sullivan <- agb_dyn_byquad(
...
# Save quadrat results 

```{r save-quad-results}
save(df_quad, df_quad_kohyama, df_quad_sullivan, file = "usedata/cachequad.rda")
```
...
### Entire plot (back to Camille's code)

We estimated the plot-level annual AGB and AWP for each method used. 
...
Save the crown-distributed quadrat-level data (added by Helene)

```{r save-crown-dist-byquad}
df_crown_2010 <- df_crown_list[[6]]
df_crown_2015 <- df_crown_list[[7]]
names(df_crown_2010)[3:5] <- names(df_crown_2015)[3:5] <- c("agb", "awp", "awm")
save(df_crown_2010,df_crown_2015, file = "usedata/cachecrown.rda")
...
save(df_plot, df_size, df_pft, df_hab, df_crown_hab, file = "usedata/cache.rda")
```
